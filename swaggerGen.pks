  ...
  '/${lower_table_name}':
    get:
      ...
      parameters:
        ...
        - $ref: "#/parameters/<%=col(i).column_name%>FilterParam"
        <% end loop;%>
      responses:
        200:
          description: Success
...
parameters:
  ...
  sortParam:
    ...
    items:
      type: string
      pattern: '[-+](<% c := col.last+1; for i in 1 .. col.last loop %><%= col(i).COLUMN_NAME %><%sep(c-i,'|'); end loop;%>)'
      default: '+<%= pk(1).column_name %>\\n'

  <% for i in 1 .. col.last loop %>
  <%=col(i).column_name%>FilterParam:
    name: <%=col(i).column_name%>\\n
    in: query
    required: false
    description: The <%=col(i).column_name%> column value to filter results.
    type: <%= col(i).data_type %>\\n
    <% if col(i).data_format is not null then %>
    format: "<%= col(i).data_format%>"
    <% end if; end loop; %>
